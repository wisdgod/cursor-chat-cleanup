...
- **TUI**(默认入口):三栏布局——左栏工作区(重名项目自动用路径消歧,
  移动光标即过滤)、中栏会话列表(只列用户手动 new 的主代理与孤儿数据)、
  右栏详情与子代理树(子代理不论层级都挂在所属主代理下);`h/l` 在三栏间移动焦点;
  按体积/行数/时间/名称/工作区排序,过滤与视图切换,多选删除/修剪/清扫/GC——
  全部两段式(预览 → 确认 → 后台执行),完成后自动刷新;
  启动即扫 key 秒出列表,体积后台补齐;
- **子代理归属**:删除主代理必须连带其全部后代子代理(含 Best-of-N 子会话),
  预览会列出连带目标;父会话已被删除的"孤儿子代理"在孤儿视图中标出,
  清扫(`x`)会把它们的数据连 header 一起清除;
- **CLI 子命令**:`report`(只读分析,含父子归属统计)、`sweep`(孤儿清扫,
  含孤儿子代理)、`gc`(blob 与 content 垃圾回收)、`delete`(按会话删除,
  默认级联子代理)、`trim`(保留正文的快照修剪)、`backup`(备份与残留检查)、
  `restore`(整体回滚)。
